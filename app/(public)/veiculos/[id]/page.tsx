...
import { FUEL_LABELS, TRANSMISSION_LABELS, WHATSAPP_NUMBER, STATUS_LABELS } from "@/lib/constants";
...
export async function generateMetadata({ params }: Params): Promise<Metadata> {
  ...
  return {
    title: `${vehicle.brand} ${vehicle.model} ${vehicle.year} — Pedro Ivo Veículos`,
...
export default async function VehiclePage({ params }: Params) {
  ...
  const msg = encodeURIComponent(
    `Olá! Tenho interesse no ${vehicle.brand} ${vehicle.model} ${vehicle.year} anunciado no site. Pode me passar mais detalhes?`
  );
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-10">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
        ...
        {/* Details */}
        <div>
          <p className="text-xs font-semibold text-[#DC2626] uppercase tracking-widest mb-1">{vehicle.brand}</p>
          <h1 className="text-3xl font-bold text-slate-900 leading-tight">
            {vehicle.model}
          </h1>
          <p className="text-slate-500 text-sm mt-1">{vehicle.year} · {vehicle.km.toLocaleString("pt-BR")} km</p>

          <div className="mt-4">
            <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium ${
              vehicle.status === "disponivel" ? "bg-emerald-50 text-emerald-700 ring-1 ring-emerald-200" :
              vehicle.status === "reservado"  ? "bg-amber-50 text-amber-700 ring-1 ring-amber-200" :
          ...
          <a
            ...
            className="mt-4 flex items-center justify-center w-full gap-2 bg-[#DC2626] hover:bg-[#B91C1C] text-white font-semibold py-3.5 rounded-xl transition-colors text-sm"
          >
